...
def spatial_pooling_module(x, prefix="ablation"):
    """
    Input x: (B, 2, 128, C)
    Remove BiLSTM + Self-Attention + Multi-Head Attention
    Keep only simple pooling for ablation
    """
    seq_len = x.shape[2]
    channels = x.shape[1] * x.shape[3]
    if seq_len is None or channels is None:
        raise ValueError("Static shape inference failed before ablation module.")

    x = Reshape((seq_len, channels), name=f"{prefix}_reshape")(x)   # (B, 128, 2*C)
    x = GlobalAveragePooling1D(name=f"{prefix}_gap")(x)             # (B, 2*C)
    return x
# -------------------------
# HANet
# -------------------------
def HANet_without_attention(input_shape=(2, 128), classes=11, dropout_rate=0.5, **kwargs):
    ...
    fusion = Conv2D(50, (2, 50), padding="same", activation="relu", name="fusion_conv")(fusion)

    spatial_output = dsc_residual_module(fusion, filters=50, prefix="global_dsc_res")
    ablation_output = spatial_pooling_module(
        spatial_output,
        prefix="final_ablation"
    )


    x = Dense(128, activation="relu", name="classifier_dense")(ablation_output)
